...
for i in os.listdir(os.getcwd()):
        if i.startswith('delta_T_v3_no_halos'):
            ...
            v2= ['f_=_', str(f) , '_MHz_', '_and_z_=_', str(z),'_signal_x=_','.pdf']
            ...
            plt.xlabel('Mpc')
            plt.ylabel('Mpc')
            plt.imshow(fs[0,:,:], cmap='jet')
            plt.colorbar()

            plt.subplot(2,2,2)
            plt.xlabel('Mpc')
            plt.ylabel('Mpc')
            plt.imshow(ff[0,:,:], cmap='jet')
            plt.colorbar()

            plt.subplot(2,2,3)
            plt.xlabel('Mpc')
            plt.ylabel('Mpc')
            plt.imshow(fk[125,:,:], cmap='jet')
            plt.colorbar()
